...
{
    if (build == NULL || uri == NULL)
    {
        log_error("Attempted to add a dependency to a NULL build object.");
        return CREN_NOK;
    }

    build_dependency_t *dependency = build_dependency_init(uri, type);
    if (dependency == NULL)
    {
        log_error("Failed to initialize build dependency.");
        return CREN_NOK;
    }

    build->dependencies = realloc(build->dependencies, sizeof(build_dependency_t *) * (build->dependencies_len + 1));
    if (build->dependencies == NULL)
    {
    ...
    build->dependencies[build->dependencies_len] = dependency;
    build->dependencies_len++;
